...
export function CoachBar({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className={`card coach${compact ? " compact" : ""}`}>
      <p className="kicker">Coach</p>
      {!compact && <p className="hint">{brain.hint}</p>}
      <div className="chips">
        <button type="button" className={`chip${listenOn ? " on" : ""}`} onPointerDown={() => void toggleListen()}>Listen</button>
        <button type="button" className="chip" onPointerDown={tap}>Tap</button>
      ...
      <p className={`conductor-meta timing-${timing}`}>
        {compact ? brain.hint : `Timing ${meter}`}
        {lastHeard ? ` · “${lastHeard}”` : ""}
        {!compact && pathNote ? ` · ${pathNote}` : ""}
        {compact && timing !== "idle" ? ` · ${meter}` : ""}
      </p>
    </section>
  );
}
